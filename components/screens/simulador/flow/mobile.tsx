...
export default function SimuladorFlowMobile({ user }: SimuladorFlowMobileProps) {
  ...
  useEffect(() => {
    if (user?.name) setNomeCompleto(user.name);
  }, [user]);
  ...
  const handleFinancingChange = (value: string) => {
    ...
    if (maxValue > 0 && financiamento > maxValue) {
      setValorFinanciamentoImovel(formatCurrency(String(Math.round(maxValue * 100))));
      return;
    }
  ...
  return (
    ...
    >
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>{simuladorFlowContent.title}</Text>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
        <View style={styles.container}>
          <Text style={styles.introText}>{simuladorFlowContent.description}</Text>
      ...
      <Modal transparent visible={loading} animationType="fade">
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", alignItems: "center" }}>
          <View style={{ backgroundColor: "#FFF", padding: 24, borderRadius: 12, elevation: 5 }}>
            <Text style={{ fontSize: 16, fontFamily: AppTheme.typography.fontBold, color: AppTheme.colors.text }}>Gerando Simulação...</Text>
          </View>
        </View>
      </Modal>
    </KeyboardAvoidingView>
